function varargout = casiairis(varargin)
% CASIAIRIS M-file for casiairis.fig
%      CASIAIRIS, by itself, creates a new CASIAIRIS or raises the existing
%      singleton*.
%
%      H = CASIAIRIS returns the handle to a new CASIAIRIS or the handle to
%      the existing singleton*.
%
%      CASIAIRIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CASIAIRIS.M with the given input arguments.
%
%      CASIAIRIS('Property','Value',...) creates a new CASIAIRIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before irissegment_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to casiairis_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help casiairis

% Last Modified by GUIDE v2.5 18-Nov-2017 11:15:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @casiairis_OpeningFcn, ...
                   'gui_OutputFcn',  @casiairis_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before casiairis is made visible.
function casiairis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to casiairis (see VARARGIN)

% Choose default command line output for casiairis
...
% UIWAIT makes casiairis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = casiairis_OutputFcn(hObject, eventdata, handles) 
...
savefile = [eyeimage_filename,'-casia.mat'];
...
resultNames = {};
i = 1;     
fid = fopen('casiadatabase.txt');
while 1
    imagename = fgetl(fid);
    if ~ischar(imagename), break, end  
eyeimage_filename = imagename;
[temp2 mask2]=createiristemplate(eyeimage_filename);
...
% createiristemplate(TEMP1,TEMP2);
